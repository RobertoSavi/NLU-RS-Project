...
def main(cfg: DictConfig):
    ...
    # Use the current working directory generated by Hydra for absolute pathing
    ...
    if cfg.testing:
        ...
        def objective(trial):
            ...
            # Merge the specific trial parameters into the config
            trial_config = OmegaConf.merge(cfg.part, trial_params)
            ...
            # Define absolute path for this specific trial's folder
            ...
            best_model, losses_train, losses_dev = train_model(
                trial_config, model, optimizer, train_loader, dev_loader, pad_index
            )
            ...
            log_path = os.path.join(trial_folder_path, "log.json")
            update_sweep_log(log_path, trial.number, trial_params, trial_ppl)
                
            # Hydra already put us in the sweep folder, so we just use the dynamic name
            losses_path = os.path.join(trial_folder_path, "losses.json")
            save_losses(losses_train, losses_dev, losses_path)
            plot_path = os.path.join(trial_folder_path, "loss_plot.png")
            plot_losses(losses_train, losses_dev, plot_path, testing=True)
            ...
            if best_val_loss < best_sweep_loss:
                ...
                best_losses_path = os.path.join(best_dir, "losses.json")
                save_losses(losses_train, losses_dev, best_losses_path)
                best_plot_path = os.path.join(best_dir, "loss_plot.png")
                plot_losses(losses_train, losses_dev, best_plot_path, testing=True)
        ...
        best_dir = os.path.join(original_cwd, "results", f"part{config.get('part', 'N/A')}", "best_model")
